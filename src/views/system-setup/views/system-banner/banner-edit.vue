            ...
            <el-card>
                <el-form label-position="right" :model="adForEdit" ref="bannerForm" :rules="rule" size="large">
                    ...
                    <el-form-item label="小程序轮播图" label-width="120px" prop="imageUrl">
                        <el-upload class="upload-demo" :action="$_baseURL + $_uploadURL" :with-credentials="true" :on-remove="handleRemove" :on-success="handleSuccess" :on-exceed="handleExceed" :file-list="fileList" list-type="picture" :limit="1" show-file-list>
...
export default {
  ...
  watch: {
    adForEdit(newValue) {
      this.fileList.push({
        name: newValue.imageUrl.substring(newValue.imageUrl.lastIndexOf('/') + 1),
        url: newValue.imageUrl
      })
    }
  },
  computed: {
  ...
  methods: {
    ...
    handleSuccess(res, file) {
      this.adForEdit.imageUrl = this.$_baseURL + res.path
    },
    // 图片上传超过限制执行的方法
    handleExceed() {
      this.$message({
